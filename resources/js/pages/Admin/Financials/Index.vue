...
const handleOrganizerPayout = (id: number, stripeAccountId: string | null) => {
    ...
    if (confirm(t('events.confirmStripeTransfer'))) {
...
const handleOrganizerRefund = (id: number) => {
    if (confirm(t('events.confirmRefundAll'))) {
        processingId.value = id;
...
const handleAffiliatePayout = (id: number) => {
    if (confirm(t('events.confirmAffiliatePayout'))) {
        processingId.value = id;
    ...
    <AppLayout>
        <div class="px-4 py-8 md:px-8 space-y-8 max-w-[1400px] mx-auto w-full min-w-0">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 class="text-3xl font-extrabold tracking-tight text-gray-900 dark:text-white">{{ t('events.adminFinancials') }}</h1>
                    <p class="text-gray-500 dark:text-gray-400 mt-1">{{ t('events.adminFinancialsDesc') }}</p>
                </div>
            </div>
            ...
            <div class="flex gap-4 border-b border-gray-200 dark:border-neutral-800 overflow-x-auto pb-px">
                ...
                >
                    {{ t('events.organizerPayouts') }} ({{ organizerPayouts.length }})
                </button>
                <button 
                ...
                >
                    {{ t('events.affiliatePayouts') }} ({{ affiliatePayouts.length }})
                </button>
                <button 
                ...
                >
                    {{ t('events.payoutHistory') }} ({{ history.length }})
                </button>
            </div>
            ...
            <div v-if="currentTab === 'organizers'" class="space-y-4">
                    ...
                    <div v-for="item in organizerPayouts" :key="item.id" 
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex flex-wrap gap-4 text-sm text-gray-500">
                                ...
                                <span class="flex items-center gap-1"><CalendarDays class="w-4 h-4" /> {{ t('events.ended') }}: {{ item.date_fin }}</span>
                            </div>
                            <div v-if="!item.organisateur.stripe_account_id" class="px-3 py-1 bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 rounded-lg inline-flex items-center gap-2 text-xs font-semibold">
                                <AlertCircle class="w-4 h-4" /> {{ t('events.noStripeAccountConnected') }}
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col md:items-end gap-3 px-4 py-3 bg-gray-50 dark:bg-neutral-800/50 rounded-2xl border border-gray-100 dark:border-neutral-700 min-w-[200px]">
                            <div class="text-xs text-gray-500 uppercase font-black">{{ t('events.netPayout') }}</div>
                            ...
                            <div class="w-full flex gap-2">
                                <button
                                    ...
                                    :title="t('events.refundParticipants')"
                                >
                ...
                <div v-else class="py-20 text-center bg-gray-50 dark:bg-neutral-800/30 rounded-3xl border-2 border-dashed border-gray-200 dark:border-neutral-800">
                    <p class="text-gray-500">{{ t('events.noOrganizerPayoutsPending') }}</p>
                </div>
            </div>
            ...
            <div v-if="currentTab === 'affiliates'" class="space-y-4">
                <div v-if="affiliatePayouts.length > 0" class="grid gap-4">
                    <div v-for="item in affiliatePayouts" :key="item.id" 
                        ...
                        <div class="flex flex-col md:items-end gap-3 px-4 py-3 bg-gray-50 dark:bg-neutral-800/50 rounded-2xl border border-gray-100 dark:border-neutral-700 min-w-[200px]">
                            <div class="text-xs text-gray-500 uppercase font-black">{{ t('events.commission') }}</div>
                ...
                <div v-else class="py-20 text-center bg-gray-50 dark:bg-neutral-800/30 rounded-3xl border-2 border-dashed border-gray-200 dark:border-neutral-800">
                    <p class="text-gray-500">{{ t('events.noAffiliatePayoutsPending') }}</p>
                </div>
            </div>
                ...
                <div v-if="history.length > 0" class="overflow-x-auto w-full">
                    <table class="w-full text-sm text-left">
                        <thead class="text-xs text-gray-500 uppercase tracking-widest bg-gray-50 dark:bg-neutral-800/50 border-b border-gray-100 dark:border-neutral-800">
                            <tr>
                                <th class="px-6 py-4">{{ t('events.type') }}</th>
                                <th class="px-6 py-4">{{ t('events.recipient') }}</th>
                                <th class="px-6 py-4">{{ t('events.event') }}</th>
                                <th class="px-6 py-4">{{ t('events.amount') }}</th>
                                <th class="px-6 py-4">{{ t('events.datePaid') }}</th>
                            </tr>
                        </thead>
                            ...
                            <tr v-for="(log, idx) in history" :key="idx" class="hover:bg-gray-50/50 dark:hover:bg-neutral-800/30">
                                <td class="px-6 py-4">
                                    <span class="px-2 py-0.5 rounded text-[10px] uppercase font-black tracking-tighter" 
                                        ...
                                        {{ log.type === 'Organizer' ? t('events.organizer') : t('events.affiliate') }}
                                    </span>
                                </td>
